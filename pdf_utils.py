...
"""
pdf_utils.py
Generacion de reportes PDF (individuales y consolidados/masivos) para la
aplicacion de Mantenimiento HVAC - Terminal de Cruceros de Amador.

--------------------------------------------------------------------------
NOTAS DE LA REESCRITURA (por que se veian desconfigurados los reportes)
--------------------------------------------------------------------------
1. Encabezado corporativo: antes se dibujaba el logo y el titulo "a mano"
   una sola vez, justo despues de add_page(). En un reporte de una sola
   pagina se veia bien, pero en cualquier reporte que ocupara 2+ paginas
   (el individual con muchas tareas/fotos, o el consolidado con varias
   ordenes) las paginas siguientes se quedaban sin encabezado, o el
   contenido de la pagina anterior podia desalinear el punto de partida.
   Ahora el encabezado se dibuja dentro del metodo header(), que fpdf2
   invoca AUTOMATICAMENTE al inicio de cada pagina (dentro de add_page()),
   antes de cualquier otro contenido. Eso garantiza una posicion identica
   del logo y el titulo en TODAS las paginas, siempre.

2. Checklist y guia de fallas: antes cada fila era un multi_cell() apilado
   manualmente, sin columnas reales. Con texto largo, una fila podia
   ocupar mas alto de lo previsto y "empujar" o solapar el siguiente
   bloque. Ahora ambos usan pdf.table(), que calcula automaticamente el
   alto de cada fila segun la celda con mas texto (envolviendo lineas) y
   mantiene las columnas perfectamente alineadas.

3. Fotos de evidencia: el bug real aqui era asumir que toda foto es
   cuadrada al reservar el espacio vertical. Una foto tomada en modo
   retrato con el celular (la orientacion mas comun) es mas alta que
   ancha, asi que el espacio reservado quedaba corto y el siguiente
   bloque se dibujaba encima de la parte baja de la foto. Ahora se lee el
   tamano real de cada imagen (con Pillow) para reservar la altura exacta.

4. Salto de pagina: ademas del salto automatico de fpdf2 (set_auto_page_break),
   se agrego un control manual antes de cada bloque de equipo o seccion
   larga (salto_si_necesario), para no dejar un titulo "huerfano" solo al
   fondo de una pagina con su contenido cortado en la siguiente.
--------------------------------------------------------------------------
"""
...
def bloque_reporte_equipo(pdf: ReportePDF, reporte: dict, modelo_info: dict,
    ...
    if reporte.get("firma_url"):
        pdf.salto_si_necesario(espacio_requerido=35)
        pdf.set_x(pdf.l_margin)
        pdf.set_font("Helvetica", "B", 9.5)
        pdf.cell(0, 5.5, "Firma del Tecnico (digital):")
        pdf.ln(6)
        _bloque_firma_imagen(pdf, reporte["firma_url"])
        pdf.ln(2)
...
def _bloque_firma_imagen(pdf: ReportePDF, firma_url: str, alto_max: float = 25, ancho_max: float = 60):
    """Incrusta la imagen de la firma digital del tecnico, respetando su
    proporcion real (misma logica que el logo del encabezado) para no
    solapar el contenido siguiente sin importar la forma de la imagen."""
    pdf.set_x(pdf.l_margin)
    resultado = _cargar_imagen(firma_url)
    if not resultado:
        pdf.set_font("Helvetica", "I", 8)
        pdf.cell(0, 5, "(no se pudo cargar la imagen de la firma)")
        pdf.ln(6)
        return
    buffer, ancho_px, alto_px = resultado
    ...
    y_inicio = pdf.get_y()
    try:
        pdf.image(buffer, x=pdf.l_margin, y=y_inicio, w=ancho_final, h=alto_final)
    except Exception:
        pdf.set_font("Helvetica", "I", 8)
        pdf.cell(0, 5, "(no se pudo cargar la imagen de la firma)")
        alto_final = 5
    pdf.set_y(y_inicio + alto_final + 2)
